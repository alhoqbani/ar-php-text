 ...
 * @link      http://www.ar-php.org
 */
class WordTag extends I18N_Arabic
{

    private $_particlePreNouns = [
        'عن', 'في', 'مذ', 'منذ',
        'من', 'الى', 'على', 'حتى',
        'الا', 'غير', 'سوى', 'خلا',
        'عدا', 'حاشا', 'ليس',
    ...
    public function tagText($str)
    {
        ...
        foreach ($text as $key => $word) {
            $text[$key][0] = $this->iconvOutput($text[$key][0]);
        }
    ...
    {
        $word = $this->iconvInput($word);
        $word_befor = $this->iconvInput($word_befor);
